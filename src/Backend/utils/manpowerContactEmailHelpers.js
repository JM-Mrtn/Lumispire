...
function getContactRecipient() {
  return cleanText(
    process.env.MANPOWER_CONTACT_RECIPIENT ||
      "crmstechalliance.work@gmail.com"
  );
}

export async function sendManpowerContactMessageEmail({
  name,
  email,
  subject,
  message,
  ...
  const { data, error } = await getResendClient().emails.send({
    ...
    subject: `Manpower Contact Message: ${cleanText(subject)}`,
    text: [
      "New LTC Manpower Services Contact Message",
      "",
      `Name: ${cleanText(name)}`,
      ...
      <div style="font-family:Arial,sans-serif;line-height:1.6;color:#1f2937;background:#f6f8f6;padding:24px;">
        <div style="max-width:640px;margin:auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e5e7eb;">
          <div style="background:#082719;color:#ffffff;padding:22px 24px;">
            <h2 style="margin:0;font-size:22px;">New LTC Manpower Services Contact Message</h2>
            <p style="margin:6px 0 0;color:#f4d484;">Lumispire Manpower Contact Form</p>
          </div>
...
export default sendManpowerContactMessageEmail;
