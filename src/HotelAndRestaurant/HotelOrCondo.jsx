...
function buildRoomCards(packages = []) {
  ...
  packages
    ...
    .forEach((pkg) => {
      ...
      if (
        Number(pkg.guestFeedback?.totalReviews || 0) >
        Number(current.guestFeedback?.totalReviews || 0)
      ) {
        current.guestFeedback = pkg.guestFeedback;
      }
...
export default function HotelOrCondo() {
  ...
  useEffect(() => {
    if (pageCount <= 1 || loadingPackages || packageError || roomCards.length === 0) {
      return undefined;
    }
        ...
        <section className="ltc-hero">
          ...
          <div className="ltc-container ltc-hero-content">
            <div>
              ...
              <p style={fontPontano}>
                Choose comfortable hotel and condo room packages with flexible durations,
                clear rates, and booking-ready room details.
              </p>
        ...
        <section className="ltc-section">
          <div className="ltc-container">
            <RevealOnScroll className="ltc-section-title">
              ...
              <h3 style={fontMontserrat}>Hotel &amp; Condo Packages</h3>
              <p style={fontPontano}>
                Browse available hotel and condo packages and check the room details before sending
              ...
              <>
                <div className="ltc-card-grid">
                  {visibleRooms.map((item, index) => (
                    <RevealOnScroll key={item._id || item.title} delay={index * 70}>
                      <ServiceCard
                        item={item}
...
function ServiceCard({ item, imageSrc, onDetails }) {
  return (
    <article
    ...
    >
      ...
      <div className="ltc-service-content">
        ...
        <GuestRating feedback={item.guestFeedback} />
      </div>
    </article>
  );
}
    ...
    <div className="ltc-modal-shell">
      ...
      <div className="ltc-modal-card">
        ...
        {Array.isArray(item.rates) && item.rates.length > 0 ? (
          <div className="ltc-rates">
            <h4 style={fontMontserrat}>Room Rates</h4>

            <div className="ltc-rate-list">
              {item.rates.map((rate) => (
                <div key={`${item.title}-modal-${rate.duration}`} className="ltc-rate-row">
                  <span style={fontMontserrat}>{rate.duration}</span>
                  <strong style={fontPontano}>{formatPeso(rate.price)}</strong>
                </div>
              ))}
            </div>
          </div>
        ...
        <GuestFeedbackSection
          packageId={item.rates?.[0]?.packageId || item.packages?.[0]?._id}
          API_BASE={API_BASE}
          summary={item.guestFeedback}
        />
