...
const pageStyles = `
  ...
  .ltc-requirement-icon {
    ...
    border-radius: 20px;
    color: var(--green-800);
    background: linear-gradient(145deg, rgba(35,95,62,.10), rgba(244,212,132,.16));
    box-shadow: inset 0 0 0 1px rgba(35,95,62,.12), 0 14px 28px rgba(8,39,25,.08);
    transition: .25s var(--ease);
  }
  ...
  .ltc-course-svg-icon svg {
    width: 40px;
    height: 40px;
    filter: drop-shadow(0 8px 10px rgba(8,39,25,.10));
  }
      ...
      <div className="ltc-sidebar-panel">
        ...
        {TRAINING_NAV_ITEMS.map((item) => (
          <button
            ...
            type="button"
            onClick={() => goTo(item.path)}
            className={`ltc-sidebar-link ${item.key === "course" ? "active" : ""}`}
            style={fontPoppins}
          >
            {item.label}
          </button>
...
function CourseIcon({ title = "" }) {
  ...
  if (name.includes("cook") || name.includes("culinary") || name.includes("food")) {
    return (
      <span className="ltc-course-svg-icon" aria-hidden="true">
        <svg viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M20 28h24l-3 24H23l-3-24Z" stroke="currentColor" strokeWidth="3.2" strokeLinejoin="round" />
          <path d="M18 28h28" stroke="currentColor" strokeWidth="3.2" strokeLinecap="round" />
          <path d="M24 24c-6-1-9-5-8-10 6 0 10 4 10 10" stroke="currentColor" strokeWidth="3.2" strokeLinecap="round" strokeLinejoin="round" />
          <path d="M32 24c0-7 4-12 10-13 2 6-2 12-10 13Z" stroke="currentColor" strokeWidth="3.2" strokeLinecap="round" strokeLinejoin="round" />
          <path d="M27 36h10" stroke="currentColor" strokeWidth="3.2" strokeLinecap="round" />
        </svg>
      </span>
    );
  }

  if (name.includes("event") || name.includes("management")) {
    return (
      <span className="ltc-course-svg-icon" aria-hidden="true">
        <svg viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M18 17h28a5 5 0 0 1 5 5v25a5 5 0 0 1-5 5H18a5 5 0 0 1-5-5V22a5 5 0 0 1 5-5Z" stroke="currentColor" strokeWidth="3.2" />
          <path d="M22 12v10M42 12v10M14 28h36" stroke="currentColor" strokeWidth="3.2" strokeLinecap="round" />
          <path d="m25 39 5 5 10-12" stroke="currentColor" strokeWidth="3.2" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </span>
    );
  }

  if (name.includes("house") || name.includes("clean")) {
    return (
      <span className="ltc-course-svg-icon" aria-hidden="true">
        <svg viewBox="0 0 64 64" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path d="M25 12h14l3 15H22l3-15Z" stroke="currentColor" strokeWidth="3.2" strokeLinejoin="round" />
          <path d="M20 27h24l4 25H16l4-25Z" stroke="currentColor" strokeWidth="3.2" strokeLinejoin="round" />
          <path d="M24 37h16M26 45h12" stroke="currentColor" strokeWidth="3.2" strokeLinecap="round" />
          <path d="M48 14c3 2 5 5 5 9" stroke="currentColor" strokeWidth="3.2" strokeLinecap="round" />
        </svg>
      </span>
    );
  }
...
export default function TrainingCourse() {
  ...
  return (
    <div className="ltc-training-home">
      ...
      <TrainingPublicHeader active="course" />

      <main>
        <TrainingPublicHero
          title="TAMSI Training"
          accent="Courses"
          description="Explore available TAMSI programs, choose your preferred course, and continue your training enrollment journey."
        />
          ...
          <div className="ltc-container">
            <div className="ltc-home-shell">
              ...
              <p className="ltc-section-intro" style={fontPoppins}>
                Select from the training and assessment courses below and continue to enrollment when you are ready.
              </p>

              {loading ? (
                <div className="ltc-course-status" style={fontPoppins}>
                  Loading courses...
                </div>
              ) : null}
              ...
              {visibleCourses.length ? (
                <div className="ltc-requirements-grid">
                  {visibleCourses.map((course, index) => {
                    const title = course.name || course.title || "Training Course";
                    const description = course.description || "Training and assessment course.";
                    return (
                      <button
                        key={course._id || course.id || title || index}
                        type="button"
                        onClick={() => goTo(TRAINING_ENROLL_ROUTE)}
                        className="ltc-requirement-card"
                      >
                        <span className="ltc-requirement-icon" aria-hidden="true">
                          <CourseIcon title={title} />
                        </span>
                        <h3 className="ltc-requirement-title" style={fontMontserrat}>{title}</h3>
                        <p className="ltc-requirement-note" style={fontPontano}>{description}</p>
                        <span className="ltc-course-action" style={fontPoppins}>Enroll Now</span>
                      </button>
                    );
                  })}
                </div>
              ) : (
                <div className="ltc-empty-card" style={fontPoppins}>
                  No training courses are available right now.
                </div>
              )}
            </div>
          </div>
