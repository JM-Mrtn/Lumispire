...
function addMessageWithoutDuplicate(prevMessages, newMessage) {
  ...
  if (newId) {
    const exists = prevMessages.some((msg) => String(msg._id || "") === newId);
...
function upsertConversation(prevConversations, incomingMessage) {
  ...
  const conversationUser =
    incomingMessage.conversationUser?._id ||
    incomingMessage.conversationUser ||
    incomingMessage.user?._id ||
    incomingMessage.user ||
    "";

  if (!conversationUser) return prevConversations;

  const conversationUserId = String(conversationUser);
  ...
  const existingIndex = prevConversations.findIndex((item) => {
    const itemUserId = item?.user?._id || item?.conversationUser || item?._id;
    return String(itemUserId || "") === conversationUserId;
  });

  if (existingIndex === -1) {
    return [
      {
        conversationUser: conversationUserId,
        user: userObject || {
        ...
        unreadCount: incomingMessage.senderRole === "user" ? 1 : 0,
      },
      ...prevConversations,
    ];
  }
  ...
  next[existingIndex] = {
    ...
    unreadCount:
      incomingMessage.senderRole === "user"
        ? Number(oldItem.unreadCount || 0) + 1
        : Number(oldItem.unreadCount || 0),
  };
...
function getMessageOwnerLabel(msg, selectedUser) {
  if (msg?.senderRole === "bot") return "Hotel Support Bot";
  if (msg?.senderRole === "admin") return "Hotel Admin";
...
export default function HotelAdminChat() {
    ...
    return [...conversations]
      .filter((conversation) => {
        if (concernFilter !== "all") {
          if (String(conversation.latestConcernType || "") !== concernFilter) {
            return false;
          }
        }
        ...
        const bookingId = String(
          conversation.latestConcernDetails?.bookingId || ""
        ).toLowerCase();
  ...
  const selectedConversation = useMemo(() => {
    return conversations.find((item) => {
      const itemUserId = item?.user?._id || item?.conversationUser || item?._id;
      return String(itemUserId || "") === String(selectedUserId);
    });
  ...
  const fetchMessages = async (userId) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/chat/admin/conversations/${userId}/messages`,
        {
          headers: adminHeaders(),
        }
      );
      ...
      setConversations((prev) =>
        prev.map((item) => {
          ...
          if (String(itemUserId || "") !== String(userId)) return item;
  ...
  const sendMessage = async (e) => {
    ...
    try {
      const res = await fetch(
        `${API_BASE}/chat/admin/conversations/${selectedUserId}/messages`,
        {
          method: "POST",
          headers: adminHeaders(),
          body: JSON.stringify({
  ...
  useEffect(() => {
    ...
    socket.on("connect", () => {
      ...
      if (selectedUserIdRef.current) {
        socket.emit("hotelChat:joinConversation", {
          userId: selectedUserIdRef.current,
        });
      }
    });

    socket.on("hotelChat:newConversationMessage", (payload) => {
      ...
      if (activeUserId && String(incomingUserId) === String(activeUserId)) {
        ...
        setConversations((prev) =>
          prev.map((item) => {
            ...
            if (String(itemUserId || "") !== String(activeUserId)) return item;
    ...
    return () => {
      socket.off("connect");
      socket.off("hotelChat:newConversationMessage");
  ...
  useEffect(() => {
    if (selectedUserId) {
      ...
      socketRef.current?.emit("hotelChat:joinConversation", {
  ...
  const isBot = (msg) => msg.senderRole === "bot";

  return (
    <HotelAdminShell
      title="Guest Conversations"
      subtitle="Filter guest chats by concern type, search by Booking ID, and reply from one support inbox."
      activePage="chat"
      maxWidth="max-w-7xl"
              ...
              {loadingConversations ? (
                ...
                sortedConversations.map((conversation) => {
                  ...
                  const conversationUserId =
                    user?._id || conversation.conversationUser || conversation._id;

                  const active = String(conversationUserId) === String(selectedUserId);
            ...
            {selectedUser ? (
                  ...
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="flex flex-wrap gap-2">
                      <span className="w-fit rounded-full bg-[#355240]/10 px-4 py-2 text-xs font-extrabold uppercase text-[#355240]">
                        Concern:{" "}
                        {getConcernLabel(selectedConversation?.latestConcernType)}
                      </span>
                ...
                >
                  <div className="flex flex-col gap-3 sm:flex-row">
                    <textarea
                      ...
                      placeholder={`Reply to ${getUserName(selectedUser)}...`}
                      rows={2}
              ...
              <div className="flex flex-1 items-center justify-center p-8 text-center">
                <div>
                  ...
                  <h2 className="mt-4 text-2xl font-extrabold text-[#355240]">
                    Select a guest
                  </h2>
                  <p className="mt-2 max-w-md text-sm font-semibold text-black/45">
                    Choose a guest conversation from the left side.
                  </p>
                </div>
              </div>
            )}
