...
function getApiBase() {
  const raw = (import.meta.env.VITE_HOTEL_API_BASE || import.meta.env.VITE_API_BASE || import.meta.env.VITE_API_URL || "http://localhost:5000").replace(/\/+$/, "");
...
function formatDateMMDDYYYY(yyyyMmDd) {
  if (!yyyyMmDd || !/^\d{4}-\d{2}-\d{2}$/.test(String(yyyyMmDd))) return yyyyMmDd || "";
...
export default function HotelBookingSummary() {
  ...
  const fullTotalAmount = Number(bookingData?.price || bookingData?.totalAmount || 0);
  ...
  const statusStyles = status.type === "success" ? "border-emerald-200 bg-emerald-50 text-emerald-700" : status.type === "error" ? "border-rose-200 bg-rose-50 text-rose-700" : "border-slate-200 bg-slate-50 text-slate-700";
  ...
  const submitBooking = async () => {
    setStatus({ type: "", message: "" });
    if (!paymentMethod) return setStatus({ type: "error", message: "Please select a payment method by clicking a QR image or choosing from the dropdown." });
    if (!proofFile) return setStatus({ type: "error", message: "Please upload proof of payment." });
    const token = localStorage.getItem("token") || localStorage.getItem("hotelToken");
    if (!token) return navigate("/hotel-login");
    setLoading(true);
    try {
      const fd = new FormData();
      fd.append("serviceType", bookingData.serviceType || "Hotel & Condo");
      fd.append("packageId", bookingData.packageId || bookingData.selectedPackageId || "");
      fd.append("packageTitle", bookingData.selectedPackageTitle || bookingData.selectedPackage || bookingData.packageTitle || "");
      fd.append("roomType", bookingData.roomType || "");
      fd.append("duration", bookingData.duration || "");
      fd.append("date", bookingData.date || "");
      fd.append("time", bookingData.time || "");
      fd.append("pax", String(bookingData.pax || ""));
      fd.append("maxPax", String(bookingData.maxPax || bookingData.pax || ""));
      fd.append("price", String(fullTotalAmount));
      fd.append("totalAmount", String(fullTotalAmount));
      fd.append("amountToPay", String(amountToPay));
      fd.append("paidAmount", String(amountToPay));
      fd.append("balanceAmount", String(balanceAmount));
      fd.append("paymentTerm", isDownPayment ? "DOWN_PAYMENT" : "FULL_PAYMENT");
      fd.append("paymentMethod", paymentMethod);
      fd.append("proof", proofFile);
      const res = await fetch(`${API_BASE}/hotel-room-bookings`, { method: "POST", headers: { Authorization: `Bearer ${token}` }, body: fd });
      ...
      if (!res.ok) {
        setStatus({ type: "error", message: data.message || "Hotel booking failed." });
        return;
      }
      sessionStorage.removeItem("hotelBookingDraft");
      setStatus({ type: "success", message: "Submitted! Waiting for admin approval." });
      setTimeout(() => navigate("/hotel-condo"), 1000);
    } catch (error) {
      ...
      <div className="min-h-screen bg-[#2f523d] font-['Inter',sans-serif]">
        ...
        <main className="flex min-h-[calc(100vh-180px)] items-center justify-center bg-[#2f523d] px-4 py-10">
          <div className="w-full max-w-md rounded-md bg-white/15 px-6 py-8 text-center shadow-[0_18px_45px_rgba(0,0,0,0.12)] backdrop-blur-[1px]">
            ...
            <button className="mt-6 h-9 rounded-full bg-white px-10 font-['Montserrat',sans-serif] text-[11px] font-extrabold uppercase text-[#3f5b44] shadow-sm transition hover:bg-[#fffde9]" onClick={() => navigate("/hotel-booking-form")} type="button">Back to Form</button>
          </div>
        </main>
        <Footer />
      </div>
      ...
      <main className="bg-[#2f523d] px-4 pb-7 pt-5 sm:px-6 lg:px-8">
        <div className="mx-auto max-w-[1180px]">
          <div className="mb-5 text-center">
            <h1 className="font-['Montserrat',sans-serif] text-[34px] font-extrabold leading-tight text-white sm:text-[42px]">Hotel Booking Summary</h1>
          ...
          <div className="rounded-md bg-white/15 px-5 py-8 shadow-[0_18px_45px_rgba(0,0,0,0.12)] backdrop-blur-[1px] sm:px-7 lg:px-8">
            ...
            <div className="mt-9">
              <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center"><div><h2 className="font-['Montserrat',sans-serif] text-[25px] font-semibold text-white sm:text-[30px]">Booking Details</h2><div className="mt-1 h-[2px] w-[210px] bg-white/60" /></div><input value={bookingData.serviceType || "Hotel & Condo"} disabled readOnly className="h-8 w-full rounded-md border-0 bg-white px-3 text-[12px] font-semibold text-[#3f5b44] outline-none sm:w-[250px]" /></div>
              <div className="grid grid-cols-1 gap-x-10 gap-y-5 md:grid-cols-3">
                <ReadOnlyField label="Package" value={bookingData.selectedPackageTitle || bookingData.packageTitle || bookingData.selectedPackage} />
                <ReadOnlyField label="Room Type" value={bookingData.roomType} />
                <ReadOnlyField label="Duration" value={bookingData.duration} />
                <ReadOnlyField label="Date" value={formatDateMMDDYYYY(bookingData.date)} />
                <ReadOnlyField label="Time" value={bookingData.time} />
                <ReadOnlyField label="Pax" value={bookingData.pax} />
                <ReadOnlyField label="Max Pax" value={bookingData.maxPax} />
                <ReadOnlyField label="Base Price" value={formatPeso(bookingData.basePrice)} />
                <ReadOnlyField label="Total Amount" value={formatPeso(fullTotalAmount)} />
              </div>
              ...
              <div className="mt-9 rounded-md bg-[#f7f7f2] px-4 py-4 text-[#314f3b]"><div className="flex items-start justify-between gap-5"><div><p className="font-['Montserrat',sans-serif] text-[20px] font-extrabold sm:text-[25px]">Total Amount:</p><div className="mt-3 space-y-2 text-sm font-semibold sm:text-base"><p>{isDownPayment ? "Down payment" : "Full Payment"}</p><p>Total Balance</p></div></div><div className="text-right"><p className="font-['Montserrat',sans-serif] text-[20px] font-extrabold sm:text-[25px]">{formatPeso(amountToPay)}</p><div className="mt-3 space-y-2 text-sm font-semibold sm:text-base"><p>{formatPeso(amountToPay)}</p><p>{formatPeso(balanceAmount)}</p></div></div></div></div>
              <div className="mt-9 flex flex-col items-center justify-center gap-4 sm:flex-row"><button onClick={submitBooking} disabled={loading} className="h-8 w-full rounded-full bg-white px-12 font-['Montserrat',sans-serif] text-[10px] font-extrabold uppercase text-[#3f5b44] shadow-sm transition hover:bg-[#fffde9] disabled:cursor-not-allowed disabled:opacity-60 sm:w-[220px]" type="button">{loading ? "Submitting..." : "Submit Booking"}</button><button onClick={() => navigate("/hotel-booking-form", { state: bookingData })} disabled={loading} className="h-8 w-full rounded-full bg-white px-12 font-['Montserrat',sans-serif] text-[10px] font-extrabold uppercase text-[#3f5b44] shadow-sm transition hover:bg-[#fffde9] disabled:cursor-not-allowed disabled:opacity-60 sm:w-[220px]" type="button">Cancel</button></div>
            </div>
          </div>
        </div>
      </main>
