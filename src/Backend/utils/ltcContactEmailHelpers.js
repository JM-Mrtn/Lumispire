...
function getFromAddress() {
  return cleanText(
    process.env.LTC_FROM_EMAIL ||
      process.env.RESEND_FROM_EMAIL ||
      "LTC Group of Companies <onboarding@resend.dev>"
  );
}

function getContactRecipient() {
  return cleanText(
    process.env.LTC_CONTACT_RECIPIENT ||
      "crmstechalliance.work@gmail.com"
  );
}

export async function sendLtcContactMessageEmail({ name, email, message }) {
  ...
  const { data, error } = await getResendClient().emails.send({
    ...
    subject: `LTC Group Contact Message from ${cleanText(name)}`,
    text: [
      "New LTC Group of Companies Contact Message",
      "",
      `Name: ${cleanText(name)}`,
      ...
      <div style="font-family:Arial,sans-serif;line-height:1.6;color:#1f2937;background:#f6f8f6;padding:24px;">
        <div style="max-width:640px;margin:auto;background:#ffffff;border-radius:16px;overflow:hidden;border:1px solid #e5e7eb;">
          <div style="background:#082719;color:#ffffff;padding:22px 24px;">
            <h2 style="margin:0;font-size:22px;">New LTC Group of Companies Contact Message</h2>
            <p style="margin:6px 0 0;color:#f4d484;">LTC Group of Companies Contact Form</p>
          </div>
...
export default sendLtcContactMessageEmail;
